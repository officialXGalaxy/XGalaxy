...
*
*   Why is this needed?
*   - In XGalaxy some blocks are superblocks, which output much higher amounts of coins
...
void CMasternodePayments::ProcessMessage(CNode* pfrom, std::string& strCommand, CDataStream& vRecv)
{
    ...
    if(fLiteMode) return; // disable all XGalaxy specific functionality
    ...
    BOOST_FOREACH(CMasternodePayee& payee, vecPayees) {
        if (payee.GetVoteCount() >= MNPAYMENTS_SIGNATURES_REQUIRED) {
            BOOST_FOREACH(CTxOut txout, txNew.vout) {
            	CScript payeeScript = payee.GetPayee();
                if (payeeScript == txout.scriptPubKey && hasMasternodePayment(payeeScript, txNew.GetValueOut(), txout.nValue, nBlockHeight)) {
