...
{
    if(fLiteMode) return; // ignore all Tank related functionality
...
/*
    ...
    allow endless transaction that would bloat Tank and make it unusable. To
...
bool CDarkSendSigner::IsVinAssociatedWithPubkey(const CTxIn& txin, const CPubKey& pubkey)
{
    ...
    if(GetTransaction(txin.prevout.hash, tx, Params().GetConsensus(), hash, true)) {
        BOOST_FOREACH(CTxOut out, tx.vout)
            if (out.nValue == 300000 * COIN && out.scriptPubKey == payee)
                return true;
    }
