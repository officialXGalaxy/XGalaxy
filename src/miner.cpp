...
// ***TODO*** ScanHash is not yet used in XGalaxy
//
...
{
    LogPrintf("XGalaxyMiner -- started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("xgalaxy-miner");
    ...
    try {
        ...
        while (true) {
            ...
            if (!pblocktemplate.get())
            {
                LogPrintf("XGalaxyMiner -- Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                return;
            }
    ...
    {
        LogPrintf("XGalaxyMiner -- terminated\n");
        throw;
    }
    catch (const std::runtime_error &e)
    {
        LogPrintf("XGalaxyMiner -- runtime error: %s\n", e.what());
        return;
    }
}
