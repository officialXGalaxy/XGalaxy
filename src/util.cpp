...
const char * const BITCOIN_CONF_FILENAME = "tank.conf";
const char * const BITCOIN_PID_FILENAME = "tankd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "tank" is a composite category enabling all Tank-related debug output
            if(ptrCategory->count(string("tank"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\TankCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\TankCore
    // Mac: ~/Library/Application Support/TankCore
    // Unix: ~/.tankcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "TankCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/TankCore";
#else
    // Unix
    return pathRet / ".tankcore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    {
        // Don't overwrite existing settings so command line settings override tank.conf
