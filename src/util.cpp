...
const char * const BITCOIN_CONF_FILENAME = "xgalaxy.conf";
const char * const BITCOIN_PID_FILENAME = "xgalaxyd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "xgalaxy" is a composite category enabling all XGalaxy-related debug output
            if(ptrCategory->count(string("xgalaxy"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\XGalaxyCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\XGalaxyCore
    // Mac: ~/Library/Application Support/XGalaxyCore
    // Unix: ~/.xgalaxycore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "XGalaxyCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/XGalaxyCore";
#else
    // Unix
    return pathRet / ".xgalaxycore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty xgalaxy.conf if it does not excist
        ...
        if (configFile != NULL) {
        	...
        	LogPrintf("xgalaxy.conf file not found or can't be created\n");
    ...
    {
        // Don't overwrite existing settings so command line settings override xgalaxy.conf
