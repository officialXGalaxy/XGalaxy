    ...
    walletModel(0)
{
    ...
    ui->tableWidgetMyMasternodes->setColumnWidth(6, columnLevelWidth);
    ...
    ui->tableWidgetMasternodes->setColumnWidth(5, columnLevelWidth);
...
void MasternodeList::updateMyMasternodeInfo(QString strAlias, QString strAddr, masternode_info_t& infoMn)
{
    ...
    if(pcoinsTip) {
		...
		if(!pcoinsTip->GetCoins(infoMn.vin.prevout.hash, coins) ||
				infoMn.vin.prevout.n >= coins.vout.size() ||
				coins.vout[infoMn.vin.prevout.n].IsNull()) {
			mnLevelStr = "N/A";
		} else {
...
void MasternodeList::updateNodeList()
{
    ...
    BOOST_FOREACH(CMasternode& mn, vMasternodes)
    {
    	...
    	if(pcoinsTip) {
			CCoins coins;
			int height = chainActive.Height();
			if(!pcoinsTip->GetCoins(mn.vin.prevout.hash, coins) ||
					mn.vin.prevout.n >= coins.vout.size() ||
					coins.vout[mn.vin.prevout.n].IsNull()) {
				mnLevelStr = "N/A";
			} else {
